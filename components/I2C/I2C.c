...
void I2C_init(void)
{
	ESP_LOGI(TAG, "Semaphore creation");
...
static esp_err_t __attribute__((unused)) i2c_master_read_slave(i2c_port_t i2c_num, uint8_t *data_rd, size_t size)
{
    ...
    i2c_master_stop(cmd);
    ret = i2c_master_cmd_begin(i2c_num, cmd, 1500 / portTICK_PERIOD_MS);
    ESP_LOGI(TAG, "i2c_master_read_slave ret value: %d", ret);
...
static esp_err_t __attribute__((unused)) i2c_master_write_slave(i2c_port_t i2c_num, uint8_t *data_wr, size_t size)
{
	...
	i2c_master_stop(cmd);
	esp_err_t ret = i2c_master_cmd_begin(i2c_num, cmd, 1500 / portTICK_PERIOD_MS);
	i2c_cmd_link_delete(cmd);
...
void i2c_test_task(void *arg)
{
    ...
    while (1) {
        ...
        xSemaphoreTake(print_mux, portMAX_DELAY);
        size_t d_size = i2c_slave_write_buffer(I2C_SLAVE_NUM, data, RW_TEST_LENGTH, 1500 / portTICK_PERIOD_MS);
        if (d_size == 0) {
        ...
        ESP_LOGI(TAG, "Preparing to give Semaphore...");
        xSemaphoreGive(print_mux);
        ...
        ESP_LOGI(TAG, "Semaphore preparing to take");
        xSemaphoreTake(print_mux, portMAX_DELAY);
        //we need to fill the slave buffer so that master can read later
        ret = i2c_master_write_slave(I2C_MASTER_NUM, data_wr, RW_TEST_LENGTH);
        if (ret == ESP_OK) {
            size = i2c_slave_read_buffer(I2C_SLAVE_NUM, data, RW_TEST_LENGTH, 1500 / portTICK_PERIOD_MS);
        }
        if (ret == ESP_ERR_TIMEOUT) {
            ESP_LOGE(TAG, "I2C Timeout");
        } else if (ret == ESP_OK) {
...
/*
esp_err_t i2c_master_sensor_test(i2c_port_t i2c_num)
{
    int ret = ESP_OK;
    uint8_t write_buf[2] = {0x6B, 0x07};

    i2c_master_write_to_device(i2c_num,ESP_SLAVE_ADDR, write_buf, sizeof(write_buf), 500 / portTICK_PERIOD_MS);
